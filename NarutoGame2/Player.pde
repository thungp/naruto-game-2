...
class Player extends MassedBeing {
  ...
  void draw() {
    scale(0.2); // original. 0.2, if you make it .9, needto translate  y = -70, to alight character to platform
    imageMode(CENTER);
    translate(0,0); // need to y = -70 if you scale origial sprite to .9
    // if the character is facing left, invert the image
    if(direction == FACING_LEFT) {
      scale(-1,1);  // I like how they use scale to reverse direction.
      translate(20, 0); // accounts for jerkiness of turning directions.
    }
